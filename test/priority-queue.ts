...
function shouldHaveEmptyState() {
    ...
    assert(pq.size() === 0, 'New queue should be empty');
...
function shouldPushAndPopBasedOnPriority() {
    ...
    let size = pq.size();
    assert(size === 4, 'Size should be 4 after pushes');
    const popped: string[] = [];
    let item: Item | undefined = undefined;
    while (item = pq.pop()) {
        popped.push(item.value);
        assert(pq.size() === size - 1, 'Size should be decreased after each pop');
        size--;
    }
...
function shouldClearItems() {
    ...
    assert(pq.size() === 2, 'Size should be 2 after pushes');
    pq.clear();
    assert(pq.size() === 0, 'Size should be 0 after clear');
